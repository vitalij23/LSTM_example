...
if __name__ == '__main__':
    ...
    sl = 2  # test amount
    input = torch.from_numpy(data[sl:, :-1])  # without first 3, without last 1
    # [100, 1000] we use 97 inputes for train and 3 for test [97, 999]
    target = torch.from_numpy(data[sl:, 1:])  # without first 1
    print("target", target.size())
    test_input = torch.from_numpy(data[:sl, :-1])  # first 3, without last
    test_target = torch.from_numpy(data[:sl, 1:])  # first 3, without first
    # GPU
